...
# ── NEW: open file with OS default app ────────────────────────────────────────
def open_file(path: str, name: str = "") -> str:
    """
    Opens a file or folder using the OS default application.
    Works on Windows (os.startfile), Mac (open), Linux (xdg-open).
    Also searches common locations if the file is not found at the given path.
    """
    # Build candidate paths to try
    candidates = []

    if name:
        # Try path + name combined
        candidates.append(_resolve_path(path) / name)
    ...
    if name:
        # Search common locations for the filename
        for folder in ["Documents", "Desktop", "Downloads", "home"]:
            candidates.append(_resolve_path(folder) / name)
    ...
    if target is None and name:
        for found in Path.home().rglob(name):
            target = found
            break

    if target is None:
        searched = name or path
        return (
            f"Could not find '{searched}'. "
            f"Try specifying the full path, e.g. C:\\Users\\YourName\\Documents\\{searched}"
        )
...
def file_controller(
    ...
    try:
        # ── NEW: open action ──────────────────────────────────────────────
        if action == "open":
            result = open_file(path=path, name=name)
